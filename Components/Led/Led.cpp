...
{}
    ...
    )
  {
    ...
    if(!on_off.isValid())
    {
        // DONE: Add an event that indicates we received an invalid argument.
    ...
    else
    {
      ...
      // DONE: Add an event that reports the state we set to blinking.
      this->log_ACTIVITY_HI_SetBlinkingState(on_off);

      // DONE: Report the blinking state via a telemetry channel.
  ...
  void Led ::parameterUpdated(FwPrmIdType id) {
    ...
    if (PARAMID_BLINK_INTERVAL == id) {
        ...
        // DONE: Add an event with, severity activity high, named BlinkIntervalSet that takes in an argument of type U32 to report the blink interval.
            ...
            NATIVE_UINT_TYPE context)
    {
        ...
        if (is_blinking)
        {
            ...
            if (this->state != new_state)
            {
                ...
                // DONE: Add an event to report the LED state (new_state).     
        ...
        else
        {
          if(this->state == Fw::On::ON)
          {
            ...
            // DONE: Add an event to report the LED state (this->state).
